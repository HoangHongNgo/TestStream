...
def execute_model(request):
    if request.method == 'POST':
        ...
        if file_serializer.is_valid():
            ...
            # Read the WebM video using imageio
            ...
            output_file_path = 'D:/Personal/TestStream/backend/nckh_backend/heartrate/videos/output.avi'
            writer = imageio.get_writer(output_file_path, 'ffmpeg', fps=fps)

            # Write each frame to the AVI file
            for frame in frames:
                writer.append_data(frame)

            writer.close()

            # if uploaded_file.name.endswith('.webm'):
            # Save the video
            ...
            # else:
            #     return Response({'detail': 'Only WEBM videos are allowed.'}, status=status.HTTP_400_BAD_REQUEST)
